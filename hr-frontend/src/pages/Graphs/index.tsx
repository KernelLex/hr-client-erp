...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  Finance: "text-indigo-700 bg-indigo-50 border-indigo-200",
...
function ChartRenderer({ chartData, height = 340, animate = true }: ChartRendererProps) {
  ...
  if (chart_type === "bar_horizontal") {
    ...
    const color = (colors && colors[vk]) ? colors[vk] : "#4F46E5"
    return (
...
function PresetCard({ preset, onLoad, isLoading }: PresetCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 hover:shadow-md hover:border-indigo-100 transition-all group cursor-pointer"
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          disabled={isLoading}
          className="flex items-center gap-1 text-xs text-indigo-600 group-hover:text-indigo-800 font-medium"
        >
...
function SavedCard({ graph, onOpen, onDelete, onExportCSV, deleting }: SavedCardProps) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-indigo-100 transition-all overflow-hidden">
      {/* Thumbnail or placeholder */}
      <div className="h-28 bg-gradient-to-br from-gray-50 to-indigo-50 flex items-center justify-center border-b border-gray-100 cursor-pointer"
        ...
        {graph.thumbnail_data ? (
          ...
          <BarChart2 className="w-10 h-10 text-indigo-200" />
        )}
      </div>
      <div className="p-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => onOpen(graph)}
            className="flex-1 flex items-center justify-center gap-1.5 text-xs bg-indigo-600 text-white py-1.5 rounded-lg hover:bg-indigo-700 transition-colors">
            ...
            <div className="lg:col-span-1 space-y-4">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <h2 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-indigo-500" /> AI Query
                </h2>
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-indigo-600 text-white py-2.5 rounded-xl text-sm font-medium hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  >
                ...
                <div className="space-y-1.5">
                  {EXAMPLE_QUERIES.map(q => (
                    <button
                      key={q}
                      onClick={() => setQuery(q)}
                      className="w-full text-left text-xs text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 px-2.5 py-1.5 rounded-lg transition-colors flex items-center gap-2"
                    >
              ...
              {currentChart ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center justify-center h-[440px] text-center p-8">
                  ...
                  <button
                    ...
                    className="mt-5 flex items-center gap-2 text-sm text-indigo-600 border border-indigo-200 px-4 py-2 rounded-xl hover:bg-indigo-50 transition-colors"
                  >
              ...
              {(["All", "Sales", "Purchase", "Finance", "Inventory"] as const).map(cat => (
                <button key={cat} onClick={() => setPresetCat(cat)}
                  className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                    presetCat === cat ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-300"
                  }`}>
                  {cat}
                </button>
              ))}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex gap-1.5 flex-wrap">
              {(["All", "Sales", "Purchase", "Finance", "Inventory", "Custom"] as const).map(cat => (
                <button key={cat} onClick={() => setSavedCat(cat)}
                  className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
                    savedCat === cat ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-300"
                  }`}>
                  {cat}
                </button>
              ))}
            ...
            <div className="ml-auto flex border border-gray-200 rounded-lg overflow-hidden">
              <button onClick={() => setViewMode("grid")}
                className={`p-2 ${viewMode === "grid" ? "bg-indigo-600 text-white" : "bg-white text-gray-400 hover:bg-gray-50"}`}>
                <LayoutGrid className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode("list")}
                className={`p-2 ${viewMode === "list" ? "bg-indigo-600 text-white" : "bg-white text-gray-400 hover:bg-gray-50"}`}>
          ...
          {filteredSaved.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              ...
              <button onClick={() => setActiveTab("generate")}
                className="inline-flex items-center gap-2 text-sm text-indigo-600 border border-indigo-200 px-4 py-2 rounded-xl hover:bg-indigo-50">
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => handleOpenSaved(g)}
                            className="flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-800 px-2.5 py-1 border border-indigo-200 rounded-lg hover:bg-indigo-50 transition-colors">
                            <Eye className="w-3 h-3" /> Open
                          </button>
