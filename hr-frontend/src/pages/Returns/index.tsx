...
export default function ReturnsPage() {
  ...
  const [vtype, setVtype] = useState<"Credit Note" | "Debit Note">("Credit Note")
  const [openVoucher, setOpenVoucher] = useState<VoucherRow | null>(null)
  if (guard) return guard

  return (
    <div className="min-h-full" style={{ background: "var(--bg-app)" }}>
      <PageHeader workspaceLabel="Vera Enterprises Workspace" title="Returns" />
      <DeptTabs
        tabs={[
          { key: "Credit Note", label: "Sales Returns (Credit Notes)" },
          { key: "Debit Note", label: "Purchase Returns (Debit Notes)" },
        ]}
        active={vtype}
